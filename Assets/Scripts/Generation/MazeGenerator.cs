...
public class MazeGenerator : MonoBehaviour
{
    ...
    [SerializeField] int numberOfMazes = 10;  // Number of mazes to generate (up to a maximum of 10)
    ...
    void Start()
    {
        ...
        numberOfMazes = Mathf.Clamp(numberOfMazes, 1, 10);  // Ensure the number of mazes is between 1 and 10
        for (int i = 0; i < numberOfMazes; i++)
        {
            Vector2Int offset = new Vector2Int(i * size.x, 0);  // Offset each maze in the x direction
            GenerateCells(offset);
            GenerateMaze(offset);
        }
    }
    ...
    void GenerateCells(Vector2Int offset)
    {
        ...
        for (int x = 0; x < size.x; x++)
        {
            ...
            {
                Vector2Int cellPosition = new Vector2Int(x, y) + offset;
    ...
    void GenerateMaze(Vector2Int offset)
    {
        ...
        for (int x = 0; x < size.x; x++)
        {
            for (int y = 0; y < size.y; y++)
            {
                if (x < size.x - 1)
                {
                    edges.Add(new Edge(new Vector2Int(x, y) + offset, new Vector2Int(x + 1, y) + offset));
                }
                if (y < size.y - 1)
                {
                    edges.Add(new Edge(new Vector2Int(x, y) + offset, new Vector2Int(x, y + 1) + offset));
                }
            }
        }
        ...
        {
            int cellIndex1 = (edge.Cell1.x - offset.x) + (edge.Cell1.y - offset.y) * size.x;
            ...
            if (ds.Find(cellIndex1) != ds.Find(cellIndex2))
            {
                ...
                RemoveWall(cells[edge.Cell1.x - offset.x, edge.Cell1.y - offset.y], cells[edge.Cell2.x - offset.x, edge.Cell2.y - offset.y]);
            }
        }
    }
